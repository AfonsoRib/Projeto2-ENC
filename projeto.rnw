...
Jackknife-after-boostrap is a method used to find the error estimate of a bootstrap estimate. After bootstrapping, we apply the jackknife to each of the newly generated samples. Here is the algorithm, after drawing the boostrap samples:
